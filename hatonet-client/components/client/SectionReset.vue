      ...
      <div class="row">
        ...
        <div class="col-12 col-md-5">
          ...
          <div v-else class="card">
            ...
            <!-- <form> -->
            <div class="form-control" :class="[{ 'has-error': errors.new_password }]">
              <label>Mật khẩu mới</label>
              <input type="password" :class="[{ 'is-invalid': errors.new_password }]" v-model="auth.new_password" />
            </div>
            <span class="text-error" v-if="errors.new_password" v-text="errors.new_password[0]"></span>
              <div class="form-control" :class="[{ 'has-error': errors.email }]">
              <label>Nhập lại mật khẩu</label>
              <input type="password" :class="[{ 'is-invalid': errors.confirm_password }]" v-model="auth.confirm_password" />
            </div>
            <span class="text-error" v-if="errors.confirm_password" v-text="errors.confirm_password[0]"></span>
  ...
  methods: {
    async handleSubmit() {
      try {
        ...
        const resp = await this.$store.dispatch('change', formData);
    ...
    async checkToken () {
      ...
      try {
        const { data } = await this.$store.dispatch('reset', formSubmit)
