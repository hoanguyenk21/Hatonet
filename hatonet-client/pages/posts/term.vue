      ...
      <div class="container">
        <div class="row">
          <div class="col-12 col-md-9">
            ...
            <div v-for="post in terms.posts" :key="post.term_id">
              <div class="box" v-if="post.term_id == terms.id">
...
export default {
  ...
  async asyncData({ $axios, query }) {
    const { data: terms } = await $axios.get(`/api/term/${query.id}`)
    const { data: categories } = await $axios.get(`/api/category`)
    return { terms, categories }
  },
  components: { WidgetCategory, WidgetAds, SectionRelatedNews },
}
</script>
