      ...
      <div class="container">
        <div class="row">
          <div class="col-12 col-md-9">
            ...
            <div v-for="post in cater.posts" :key="post.id">
              <div class="box">
                ...
                <div class="box__content">
                  <div class="content">
                    <nuxt-link :to="{ path: `/posts/${post.id}`, query: { id: post.id } }" v-if="post.title"
                      v-text="post.title">
                    </nuxt-link>
...
export default {
  ...
  components: { WidgetCategory, WidgetAds, SectionRelatedNews,SectionImage },
  async asyncData({ $axios, query }) {
    const { data: cater } = await $axios.get(`/api/category/${query.id}`)
    const { data: categories } = await $axios.get(`/api/category`);
    return { cater, categories }
  }
}
</script>
